...
const goBackTab=()=>{const current=tabRef.current;const submenu=["Settings","ProfileSetup","FontSize","Security","LeagueRules","TrophyCase","Updates","ModeratorServer","ManagerServer","Contact","PlayerStats","DivisionPicker","ManagerTournament"].includes(current);if(!submenu)return;const prev=navBackRef.current.pop();if(!prev)return;navForwardRef.current.push(current);setTabRaw(prev)};
...
useEffect(()=>{
  ...
  const sub=BackHandler.addEventListener("hardwareBackPress",()=>{
    ...
    const submenu=["Settings","ProfileSetup","FontSize","Security","LeagueRules","TrophyCase","Updates","Contact","PlayerStats","DivisionPicker","ManagerTournament"].includes(current);
...
<Text style={[s.settingsSub,{textAlign:"center",marginTop:12,marginBottom:8}]}>{Platform.OS==="web"?"WEB v0.7.25":"APP v0.7.25"} • LIVE SERVER STATS</Text>
</ScrollView>;
