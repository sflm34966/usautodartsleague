...
function managerTournamentDateOptions(){
  const out:any[]=[];
  const base=new Date();
...
function ManagerServerPanel({currentPlayer,onBack}:{currentPlayer:any;onBack:()=>void}){
...
const refreshServerData=async()=>{
  try{
    ...
    const[divisions,announcements,matches,seasonsPayload]=await Promise.all([apiRequest("/api/divisions"),apiRequest("/api/announcements"),apiRequest("/api/matches"),apiRequest("/api/seasons")]);
    const divs=Array.isArray(divisions)?divisions:[];setServerDivisions(divs);if(Array.isArray(announcements))setServerAnnouncements(announcements);if(Array.isArray(matches))setServerMatches(matches);
    ...
    try{const awards=await apiRequest("/api/awards");const earned=Array.isArray(awards)?awards:Array.isArray(awards?.earned)?awards.earned:[];setServerEarnedAwards(earned);const name=String(currentPlayer?.display_name||currentPlayer?.username||"");if(name)playerAwards[name]=earned.map((a:any)=>({type:String(a.type||a.code||"award"),count:Number(a.count||1)}));if(Array.isArray(awards?.catalog)){const cat=normalizeAwardCatalog(awards.catalog);if(cat.length){setServerAwardCatalog(cat);await storage.setItemAsync("league_award_catalog_cache",JSON.stringify(cat));}}}catch{}
...
const changeOwnPassword=async()=>{
  ...
  try{
    ...
    if(!auth?.token||!auth?.player){
      ...
      auth=await apiRequest("/api/auth/login",{method:"POST",body:JSON.stringify({username,password:newPassword})});
    }
...
const pickMatchScreenshots=async(matchId:string)=>{
  ...
  if(!result.canceled&&result.assets?.length){
    ...
    try{
      ...
      Alert.alert("Screenshot was NOT uploaded",`The image is still selected on this phone, but the league server did not receive it. ${String(e?.message||e||"")}`);
...
const matchResultForMe=(m:any)=>{const me=Number(currentPlayer?.id||0),p1=Number(m.player1_id||0),p2=Number(m.player2_id||0);const a=Number(m.player1_legs??0),b=Number(m.player2_legs??0);if(!me||(!p1&&!p2)||a===b)return"";const won=p1===me?a>b:p2===me?b>a:false;return won?"W":"L";};
let body:any;
if(tab==="Standings"){
...
else if(tab==="Matches"){const seasonOptions=serverSeasons.map((x:any)=>({value:String(x.id),label:`${x.name} (${x.year})${Number(x.id)===currentSeasonId?" • Current":""}`}));body=<ScrollView showsVerticalScrollIndicator={false}><Text style={s.title}>My Matches</Text><ChoiceDropdown label="Season" value={selectedSeasonId} options={seasonOptions} onChange={setSelectedSeasonId} placeholder="Current Season"/><View style={s.managerNotice}><Text style={s.managerNoticeTitle}>RESULT SCREENSHOT EVIDENCE</Text><Text style={s.managerNoticeText}>Use the original AutoDarts result screenshot captured on the PC running AutoDarts. When you select it here, the app uploads the image directly to the league server and confirms receipt.</Text></View><Text style={s.statsSection}>UPCOMING</Text>{upcomingMatches.length?upcomingMatches.map((m:any)=>{const key=String(m.id);const opponent=neutralOpponent(m,currentPlayer?.id);return <View key={key} style={s.matchCard}><View style={s.matchCardRow}><View style={s.matchCardInfo}><Text style={s.matchDivision}>{String(m.division_name||currentPlayer?.division_name||"LEAGUE MATCH").toUpperCase()} • WEEK {m.week_no||"—"}</Text><View style={s.matchPlayersLeft}><Text style={s.matchSelfName}>{currentPlayer?.display_name||currentPlayer?.username||"YOU"}</Text><Text style={s.vsLarge}> VS </Text><PlayerLink name={String(opponent||"Opponent")}/></View><Text style={s.matchDateLeft}>{localMatchLabel(m,timeZone)}</Text><Text style={s.matchMetaLeft}>{m.stage&&m.stage!=="regular_season"?String(m.round_label||m.stage):"Regular Season"} • Best of {m.best_of||5} • Server scheduled</Text>{Number(m.evidence_count||0)>0?<Text style={s.uploadReady}>✓ SERVER RECEIVED {Number(m.evidence_count||0)} screenshot{Number(m.evidence_count||0)===1?"":"s"}</Text>:matchScreenshots[key]?.length?<Text style={s.uploadReady}>Selected locally — waiting for server receipt</Text>:null}</View><Pressable style={[s.uploadResultsButton,matchUploadWorkingId===key&&{opacity:.55}]} disabled={!!matchUploadWorkingId} onPress={()=>pickMatchScreenshots(key)}><Text style={s.uploadResultsIcon}>{matchUploadWorkingId===key?"…":"⬆"}</Text><Text style={s.uploadResultsText}>{matchUploadWorkingId===key?"UPLOADING":"UPLOAD"}</Text><Text style={s.uploadResultsText}>EVIDENCE</Text></Pressable></View></View>}):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO UPCOMING MATCHES</Text><Text style={s.emptyStateText}>Server-generated matches for this season will appear here automatically.</Text></View>}<Text style={s.statsSection}>COMPLETED / HISTORY</Text>{completedMatches.length?completedMatches.map((m:any)=>{const me=Number(currentPlayer?.id||0),p1=Number(m.player1_id)===me;const opponent=p1?m.player2_name:m.player1_name;const myScore=p1?m.player1_legs:m.player2_legs;const oppScore=p1?m.player2_legs:m.player1_legs;const result=matchResultForMe(m);return <View key={String(m.id)} style={s.matchCard}><View style={s.matchCardRow}><View style={s.matchCardInfo}><Text style={s.matchDivision}>{String(m.division_name||currentPlayer?.division_name||"LEAGUE MATCH").toUpperCase()} • WEEK {m.week_no||"—"}</Text><View style={s.matchPlayersLeft}><Text style={[s.matchResultLetter,result==="L"&&{color:"#ff5d67"}]}>{result||"•"}</Text><Text style={s.matchSelfName}>{currentPlayer?.display_name||currentPlayer?.username||"YOU"}</Text><Text style={s.vsLarge}> {myScore??"–"} - {oppScore??"–"} </Text><PlayerLink name={String(opponent||"Opponent")}/></View><Text style={s.matchDateLeft}>{localMatchLabel(m,timeZone)}</Text><Text style={s.matchMetaLeft}>Completed • Best of {m.best_of||5} • Stored on league server • Evidence {Number(m.evidence_count||0)}</Text>{Number(m.evidence_count||0)>0?<Text style={s.uploadReady}>✓ SERVER RECEIVED {Number(m.evidence_count||0)} screenshot{Number(m.evidence_count||0)===1?"":"s"}</Text>:null}</View><Pressable style={[s.uploadResultsButton,matchUploadWorkingId===String(m.id)&&{opacity:.55}]} disabled={!!matchUploadWorkingId} onPress={()=>pickMatchScreenshots(String(m.id))}><Text style={s.uploadResultsIcon}>{matchUploadWorkingId===String(m.id)?"…":"⬆"}</Text><Text style={s.uploadResultsText}>{matchUploadWorkingId===String(m.id)?"UPLOADING":"ADD"}</Text><Text style={s.uploadResultsText}>EVIDENCE</Text></Pressable></View></View>}):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO COMPLETED MATCHES</Text><Text style={s.emptyStateText}>Completed matches for this season will stay stored on the server.</Text></View>}</ScrollView>;}
...
else if(tab==="ManagerServer")body=<ManagerServerPanel currentPlayer={currentPlayer} onBack={()=>setTab("Settings")}/>;
