...
const leagueRules=[
  ...
  {n:"18",title:"Player Statistics",icon:"▥",items:["The server maintains season and career statistics, including W/L, legs, averages, checkout percentage, highest checkout, 60+, 100+, 140+, 170+, 180s, streaks, and match history.","Voided matches don't count."]},
...
function ManagerServerPanel({currentPlayer,onBack,onOpenPlayer}:{currentPlayer:any;onBack:()=>void;onOpenPlayer:(name:string,playerId:number)=>void}){
  ...
  if(area==="Create Tournament"){
    ...
    const deleteTournament=()=>{
      ...
      Alert.alert("Delete Tournament",`Delete ${name}? This permanently removes the tournament, bracket, matches, evidence, OCR/review data, tournament-only stats, signup responses, and tournament announcement. Tournament stats are recalculated afterward.`,[
...
const[serverTournamentStats,setServerTournamentStats]=useState<any>({});
const[serverTournamentHistory,setServerTournamentHistory]=useState<any[]>([]);
const[statsView,setStatsView]=useState<"League"|"Tournament">("League");
...
const[selectedPlayerTournamentStats,setSelectedPlayerTournamentStats]=useState<any>({});
const[selectedPlayerTournamentHistory,setSelectedPlayerTournamentHistory]=useState<any[]>([]);
const[selectedPlayerAwards,setSelectedPlayerAwards]=useState<any[]>([]);
const[selectedPlayerProfileTab,setSelectedPlayerProfileTab]=useState<"Overview"|"Match History"|"Achievements">("Overview");
const[selectedPlayerStatsMode,setSelectedPlayerStatsMode]=useState<"League"|"Tournament">("League");
const[selectedPlayerMenuOpen,setSelectedPlayerMenuOpen]=useState(false);
...
const refreshOwnStats=async(seasonId:number|string=0)=>{
  try{
    ...
    const tournament=(career?.tournament_stats&&typeof career.tournament_stats==="object")?career.tournament_stats:(season?.tournament_stats&&typeof season.tournament_stats==="object")?season.tournament_stats:{};
    setServerTournamentStats(tournament);
    const history=Array.isArray(career?.tournament_history)?career.tournament_history:Array.isArray(tournament?.history)?tournament.history:[];
    setServerTournamentHistory(history);
  }catch{}
};
...
const playerIdForName=(name:string)=>{
  ...
  if(!needle)return 0;
  const ownName=String(currentPlayer?.username||currentPlayer?.display_name||"").trim().toLowerCase();
  if(needle===ownName&&Number(currentPlayer?.id||0)>0)return Number(currentPlayer.id);
  const publicPlayer=serverPlayers.find((r:any)=>[r?.username,r?.display_name].some((v:any)=>String(v??"").trim().toLowerCase()===needle));
...
const refreshSelectedPlayerStats=async(name:string,seasonId:number|string=0,explicitPlayerId:number=0)=>{
  ...
  if(pid<=0){setSelectedPlayerSeasonStats({});setSelectedPlayerCareerStats({});setSelectedPlayerTournamentStats({});setSelectedPlayerTournamentHistory([]);setSelectedPlayerStatsError("Player stats could not be matched to a server player ID.");return;}
  ...
  try{
    ...
    const careerLeague=career?.league_stats&&typeof career.league_stats==="object"?career.league_stats:career;
    const seasonLeague=season?.league_stats&&typeof season.league_stats==="object"?season.league_stats:season;
    const tournament=career?.tournament_stats&&typeof career.tournament_stats==="object"?career.tournament_stats:{};
    const tournamentHistory=Array.isArray(career?.tournament_history)?career.tournament_history:Array.isArray(tournament?.history)?tournament.history:[];
    setSelectedPlayerCareerStats(careerLeague&&typeof careerLeague==="object"?careerLeague:{});
    setSelectedPlayerSeasonStats(seasonLeague&&typeof seasonLeague==="object"?seasonLeague:{});
    setSelectedPlayerTournamentStats(tournament);
    setSelectedPlayerTournamentHistory(tournamentHistory);
  }catch(e:any){
    setSelectedPlayerSeasonStats({});
    setSelectedPlayerCareerStats({});
    setSelectedPlayerTournamentStats({});
    setSelectedPlayerTournamentHistory([]);
...
const awardRowsToMarks=(rows:any[])=>rows.filter((a:any)=>String(a?.type||a?.code||"").toLowerCase()!=="bestleg").map((a:any)=>({type:String(a.type||a.code||"award"),count:Number(a.count||1)}));
const cacheAwardsForPlayers=(rows:any[],players:any[]=serverPlayers)=>{
  const namesById=new Map<number,string>();
  for(const p of players||[]){const pid=Number(p?.id||0);const name=String(p?.username||p?.display_name||"");if(pid>0&&name)namesById.set(pid,name);}
  if(Number(currentPlayer?.id||0)>0){const ownName=String(currentPlayer?.username||currentPlayer?.display_name||"");if(ownName)namesById.set(Number(currentPlayer.id),ownName);}
...
const refreshSelectedPlayerAwards=async(name:string,explicitPlayerId:number=0)=>{
  const pid=Number(explicitPlayerId||playerIdForName(name));
  if(pid<=0){setSelectedPlayerAwards([]);return;}
  try{const rows=await apiRequest(`/api/awards?player_id=${encodeURIComponent(String(pid))}`);if(Array.isArray(rows)){const filtered=rows.filter((a:any)=>String(a?.type||"").toLowerCase()!=="bestleg");playerAwards[name]=awardRowsToMarks(filtered);setSelectedPlayerAwards(filtered);setAwardRevision(v=>v+1);}}catch{setSelectedPlayerAwards([]);}
};
...
const refreshServerData=async()=>{
  try{
    ...
    try{const leagueAwards=await apiRequest("/api/awards?scope=league");if(Array.isArray(leagueAwards)){cacheAwardsForPlayers(leagueAwards,Array.isArray(playersPayload)?playersPayload:[]);const own=leagueAwards.filter((a:any)=>Number(a?.player_id||0)===Number(currentPlayer?.id||0)&&String(a?.type||"").toLowerCase()!=="bestleg");setServerEarnedAwards(own);}}catch{try{const awards=await apiRequest("/api/awards");const earned=(Array.isArray(awards)?awards:Array.isArray(awards?.earned)?awards.earned:[]).filter((a:any)=>String(a?.type||"").toLowerCase()!=="bestleg");setServerEarnedAwards(earned);const name=String(currentPlayer?.username||currentPlayer?.display_name||"");if(name){playerAwards[name]=awardRowsToMarks(earned);setAwardRevision(v=>v+1);}}catch{}}
...
const openPlayer=(name:string,playerId:number=0)=>{setSelectedPlayerSeasonStats({});setSelectedPlayerCareerStats({});setSelectedPlayerTournamentStats({});setSelectedPlayerTournamentHistory([]);setSelectedPlayerAwards([]);setSelectedPlayerStatsError("");setSelectedPlayerProfileTab("Overview");setSelectedPlayerStatsMode("League");setSelectedPlayerMenuOpen(false);setSelectedPlayerId(Number(playerId)||0);setSelectedPlayer(name);setTab("PlayerStats")};
...
const publicPlayerNameById=(pid:number)=>{const p=serverPlayers.find((x:any)=>Number(x?.id||0)===Number(pid));return String(p?.username||p?.display_name||"")};
let body:any;
if(tab==="Standings"){
...
const historyOptions=[
  ...
  ...serverPlayers.filter((p:any)=>p?.active!==false&&Number(p?.id||0)!==me).sort((a:any,b:any)=>String(a?.username||a?.display_name||"").localeCompare(String(b?.username||b?.display_name||""))).map((p:any)=>({value:`player:${Number(p.id)}`,label:String(p.username||p.display_name||`Player ${p.id}`)})),
...
const selectedIsMe=String(selectedPlayer).toLowerCase()===String(currentPlayer?.username||currentPlayer?.display_name||"").toLowerCase();
...
const profileTournamentStats=selectedIsMe?serverTournamentStats:selectedPlayerTournamentStats;
const profileTournamentHistory=selectedIsMe?serverTournamentHistory:selectedPlayerTournamentHistory;
const profileTZ=String(selectedIsMe?(currentPlayer?.timezone||timeZone):(profilePlayerRecord?.timezone||profilePlayerRecord?.time_zone||""));
const profileTZLabel=US_TIME_ZONES.find(z=>z.value===profileTZ)?.label||profileTZ||"—";
const recentSource=serverLeagueHistoryMatches.length?serverLeagueHistoryMatches:completedMatches;
const profileMatchHistory=recentSource.filter((m:any)=>Number(m?.player1_id||0)===profilePlayerId||Number(m?.player2_id||0)===profilePlayerId).sort((a:any,b:any)=>String(b?.scheduled_at||"").localeCompare(String(a?.scheduled_at||"")));
const recent=profileMatchHistory.slice(0,5);
const earnedRows=(selectedIsMe?serverEarnedAwards:selectedPlayerAwards).filter((a:any)=>String(a?.type||"").toLowerCase()!=="bestleg");
const mergedAward=(e:any)=>{const cat=serverAwardCatalog.find((a:any)=>String(a?.type||"")===String(e?.type||""))||{};return{...cat,...e,type:String(e?.type||cat?.type||"award"),name:String(e?.name||cat?.name||e?.type||"Award"),detail:String(e?.detail||cat?.detail||"Award earned and stored on the league server."),category:String(e?.category||cat?.category||"Earned Award"),count:Number(e?.count||1)};};
const renderProfileMatch=(m:any)=>{const isP1=Number(m.player1_id)===profilePlayerId;const opponentId=isP1?Number(m.player2_id||0):Number(m.player1_id||0);const opponent=publicPlayerNameById(opponentId)||String(isP1?m.player2_name:m.player1_name||"Opponent");const myScore=isP1?m.player1_legs:m.player2_legs;const oppScore=isP1?m.player2_legs:m.player1_legs;const result=matchResultForPlayer(m,profilePlayerId);return <View key={String(m.id)} style={s.profileMatchRow}><View style={[s.profileResultBadge,result==="L"&&s.profileResultLoss]}><Text style={s.profileResultText}>{result||"•"}</Text></View><View style={s.profileOpponentAvatar}><Text style={s.profileOpponentInitial}>{String(opponent||"?")[0]}</Text></View><View style={s.profileOpponentInfo}><PlayerLink name={String(opponent||"Opponent")} playerId={opponentId} textStyle={s.profileOpponentName}/><Text style={s.profileOpponentSub}>{String(m?.division_name||"League match")}</Text></View><Text style={[s.profileScore,result==="L"&&{color:"#ff453a"}]}>{myScore??"–"} - {oppScore??"–"}</Text><View style={s.profileMatchDateWrap}><Text style={s.profileMatchDate}>{localMatchLabel(m,timeZone)}</Text><Text style={s.profileMatchType}>Completed</Text></View></View>;};
body=<ScrollView showsVerticalScrollIndicator={false}>
<View style={s.profileHeaderRow}><Pressable onPress={()=>setTab("Standings")} hitSlop={8}><Text style={s.profileBack}>‹</Text></Pressable><Text style={s.profilePageTitle}>PLAYER PROFILE</Text>{!selectedIsMe?<Pressable onPress={()=>setSelectedPlayerMenuOpen(v=>!v)} hitSlop={10} accessibilityRole="button" accessibilityLabel="Player profile options"><Text style={s.profileMore}>•••</Text></Pressable>:<View style={{width:34}}/>}</View>
{!selectedIsMe&&selectedPlayerMenuOpen?<View style={s.profileMoreMenu}><Pressable style={[s.profileMoreMenuItem,selectedPlayerStatsMode==="League"&&s.profileMoreMenuItemActive]} onPress={()=>{setSelectedPlayerStatsMode("League");setSelectedPlayerProfileTab("Overview");setSelectedPlayerMenuOpen(false)}}><Text style={s.profileMoreMenuTitle}>LEAGUE OVERVIEW</Text><Text style={s.profileMoreMenuSub}>Season and career league statistics</Text></Pressable><Pressable style={[s.profileMoreMenuItem,selectedPlayerStatsMode==="Tournament"&&s.profileMoreMenuItemActive]} onPress={()=>{setSelectedPlayerStatsMode("Tournament");setSelectedPlayerProfileTab("Overview");setSelectedPlayerMenuOpen(false)}}><Text style={s.profileMoreMenuTitle}>TOURNAMENT STATS</Text><Text style={s.profileMoreMenuSub}>Manager-created tournament statistics and history</Text></Pressable></View>:null}
...
<View style={s.profileMetaRow}><View style={s.profileMetaItem}><Text style={s.profileMetaIcon}>🛡</Text><Text style={s.profileMetaLabel}>DIVISION</Text><Text style={s.profileMetaValue}>{selectedIsMe?(currentPlayer?.division_name||"—"):(profilePlayerRecord?.division_name||live?.division_name||"—")}</Text></View><View style={s.profileMetaDivider}/><View style={s.profileMetaItem}><Text style={s.profileMetaIcon}>▥</Text><Text style={s.profileMetaLabel}>RANK</Text><Text style={s.profileMetaValue}>{live?.rank?`#${live.rank}`:"—"}</Text></View><View style={s.profileMetaDivider}/><View style={s.profileMetaItem}><Text style={s.profileMetaIcon}>▣</Text><Text style={s.profileMetaLabel}>MATCHES</Text><Text style={s.profileMetaValue}>{played}</Text></View><View style={s.profileMetaDivider}/><View style={s.profileMetaItem}><Text style={s.profileMetaIcon}>◷</Text><Text style={s.profileMetaLabel}>TIME ZONE</Text><Text style={s.profileMetaValue}>{profileTZLabel}</Text></View></View>
{!selectedIsMe&&selectedPlayerStatsMode==="Tournament"?<View style={s.profileModeBanner}><Text style={s.profileModeBannerTitle}>TOURNAMENT STATS</Text><Text style={s.profileModeBannerSub}>Separate from official league statistics</Text></View>:null}
<View style={s.profileTabs}>{(["Overview","Match History","Achievements"] as const).map(x=><Pressable key={x} onPress={()=>setSelectedPlayerProfileTab(x)} style={s.profileTabButton}><Text style={[s.profileTab,selectedPlayerProfileTab===x&&s.profileTabActive]}>{x.toUpperCase()}</Text></Pressable>)}</View>
{selectedPlayerProfileTab==="Overview"?(selectedPlayerStatsMode==="Tournament"&&!selectedIsMe?<>
<View style={s.profileSectionCard}><View style={s.profileSectionHeader}><Text style={s.profileSectionTitle}>TOURNAMENT RECORD</Text><Text style={s.profileSeasonYear}>SERVER DATA</Text></View><View style={s.profileStatsMatrix}>{[["TOURNAMENTS",formatStatInt(profileTournamentStats.tournaments_played),"#ffffff"],["TITLES",formatStatInt(profileTournamentStats.tournament_wins),"#f2c94c"],["MATCHES",formatStatInt(profileTournamentStats.matches_played),"#ffffff"],["WINS",formatStatInt(profileTournamentStats.matches_won),"#35c759"],["LOSSES",formatStatInt(profileTournamentStats.matches_lost),"#ff5d67"],["LEGS WON",formatStatInt(profileTournamentStats.legs_won),"#35c759"],["LEGS LOST",formatStatInt(profileTournamentStats.legs_lost),"#ff5d67"],["3-DART AVG",selectedPlayerStatsLoading?"…":formatStat(profileTournamentStats.three_dart_average),"#2f80ed"],["AVG UNTIL 170",selectedPlayerStatsLoading?"…":formatStat(profileTournamentStats.average_until_170),"#2f80ed"],["FIRST 9",selectedPlayerStatsLoading?"…":formatStat(profileTournamentStats.first9_average),"#f2c94c"],["CHECKOUT %",selectedPlayerStatsLoading?"…":`${formatStat(profileTournamentStats.checkout_pct,1)}%`,"#2ec4b6"],["CHECKOUTS",selectedPlayerStatsLoading?"…":`${formatStatInt(profileTournamentStats.checkouts_made)}/${formatStatInt(profileTournamentStats.checkout_attempts)}`,"#35c759"],["HIGH FINISH",selectedPlayerStatsLoading?"…":formatStatInt(profileTournamentStats.highest_checkout),"#ff453a"],["60+",formatStatInt(profileTournamentStats["60_plus"]),"#ffffff"],["100+",formatStatInt(profileTournamentStats["100_plus"]),"#ffffff"],["140+",formatStatInt(profileTournamentStats["140_plus"]),"#ffffff"],["170+",formatStatInt(profileTournamentStats["170_plus"]),"#ffffff"],["180",formatStatInt(profileTournamentStats["180s"]),"#ffffff"]].map((x:any,i:number)=><View key={i} style={s.profileStatBox}><Text style={s.profileStatLabel}>{x[0]}</Text><Text style={[s.profileStatValue,{color:x[2]}]}>{x[1]}</Text></View>)}</View><Text style={s.statsNote}>{selectedPlayerStatsError?`SERVER STATS ERROR • ${selectedPlayerStatsError}`:`TOURNAMENT-ONLY SERVER STATS • ${formatStatInt(profileTournamentStats.matches_with_stats)} approved tournament match${Number(profileTournamentStats.matches_with_stats||0)===1?"":"es"} with statistics.`}</Text></View>
<View style={s.profileSectionCard}><View style={s.profileRecentHeader}><Text style={s.profileSectionTitle}>TOURNAMENT HISTORY</Text></View>{profileTournamentHistory.length?profileTournamentHistory.map((t:any,i:number)=><View key={String(t?.tournament_id||i)} style={s.profileTournamentHistoryRow}><View style={{flex:1}}><Text style={s.profileTournamentName}>{String(t?.name||"Tournament")}</Text><Text style={s.profileTournamentMeta}>{String(t?.placement||"Played")} • {formatStatInt(t?.matches_won)}-{formatStatInt(t?.matches_lost)} matches • Legs {formatStatInt(t?.legs_won)}-{formatStatInt(t?.legs_lost)}</Text></View><Text style={[s.profileTournamentPlacement,t?.champion&&{color:"#f2c94c"}]}>{String(t?.placement||"").toUpperCase()}</Text></View>):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO TOURNAMENT HISTORY</Text><Text style={s.emptyStateText}>Completed manager-created tournament appearances will appear here.</Text></View>}</View>
</>:<>
<View style={s.profileSectionCard}><View style={s.profileSectionHeader}><Text style={s.profileSectionTitle}>SEASON STATS</Text><Text style={s.profileSeasonYear}>SERVER DATA</Text></View><View style={s.profileStatsMatrix}>{[["MATCHES",String(played),"#ffffff"],["WINS",String(wins),"#35c759"],["LOSSES",String(losses),"#ff5d67"],["WIN %",winPct,"#9b6cff"],["3-DART AVG",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStat(profileSeasonStats.three_dart_average),"#2f80ed"],["FIRST 9",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStat(profileSeasonStats.first9_average),"#f2c94c"],["CHECKOUT %",selectedPlayerStatsLoading&&!selectedIsMe?"…":`${formatStat(profileSeasonStats.checkout_pct,1)}%`,"#2ec4b6"],["HIGH FINISH",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStatInt(profileSeasonStats.highest_checkout),"#ff453a"],["CHECKOUTS",selectedPlayerStatsLoading&&!selectedIsMe?"…":`${formatStatInt(profileSeasonStats.checkouts_made)}/${formatStatInt(profileSeasonStats.checkout_attempts)}`,"#35c759"]].map((x:any,i:number)=><View key={i} style={s.profileStatBox}><Text style={s.profileStatLabel}>{x[0]}</Text><Text style={[s.profileStatValue,{color:x[2]}]}>{x[1]}</Text></View>)}</View><Text style={s.statsNote}>{selectedPlayerStatsError&&!selectedIsMe?`SERVER STATS ERROR • ${selectedPlayerStatsError}`:`LIVE SERVER STATS • ${formatStatInt(profileSeasonStats.matches_with_stats)} approved match${Number(profileSeasonStats.matches_with_stats||0)===1?"":"es"} with imported dart statistics.`}</Text></View>
<View style={s.profileSectionCard}><View style={s.profileSectionHeader}><Text style={s.profileSectionTitle}>CAREER STATS</Text><Text style={s.profileSeasonYear}>SERVER DATA ONLY</Text></View><View style={s.profileStatsMatrix}>{[["MATCHES",String(played),"#ffffff"],["WINS",String(wins),"#35c759"],["LOSSES",String(losses),"#ff5d67"],["WIN %",winPct,"#9b6cff"],["3-DART AVG",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStat(profileCareerStats.three_dart_average),"#2f80ed"],["FIRST 9",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStat(profileCareerStats.first9_average),"#f2c94c"],["CHECKOUT %",selectedPlayerStatsLoading&&!selectedIsMe?"…":`${formatStat(profileCareerStats.checkout_pct,1)}%`,"#2ec4b6"],["HIGH FINISH",selectedPlayerStatsLoading&&!selectedIsMe?"…":formatStatInt(profileCareerStats.highest_checkout),"#ff453a"],["CHECKOUTS",selectedPlayerStatsLoading&&!selectedIsMe?"…":`${formatStatInt(profileCareerStats.checkouts_made)}/${formatStatInt(profileCareerStats.checkout_attempts)}`,"#35c759"]].map((x:any,i:number)=><View key={i} style={s.profileStatBox}><Text style={s.profileStatLabel}>{x[0]}</Text><Text style={[s.profileStatValue,{color:x[2]}]}>{x[1]}</Text></View>)}</View></View>
<View style={s.profileSectionCard}><View style={s.profileRecentHeader}><Text style={s.profileSectionTitle}>RECENT MATCHES</Text><Pressable onPress={()=>setSelectedPlayerProfileTab("Match History")}><Text style={s.profileViewAll}>VIEW ALL</Text></Pressable></View>{recent.length?recent.map(renderProfileMatch):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO MATCH HISTORY AVAILABLE</Text><Text style={s.emptyStateText}>Completed server matches for this player and season will appear here.</Text></View>}</View>
</>):null}
{selectedPlayerProfileTab==="Match History"?<View style={s.profileSectionCard}><View style={s.profileRecentHeader}><Text style={s.profileSectionTitle}>MATCH HISTORY</Text><Text style={s.profileSeasonYear}>{profileMatchHistory.length} MATCH{profileMatchHistory.length===1?"":"ES"}</Text></View>{profileMatchHistory.length?profileMatchHistory.map(renderProfileMatch):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO MATCH HISTORY AVAILABLE</Text><Text style={s.emptyStateText}>No completed league matches are stored for this player in the selected season.</Text></View>}</View>:null}
{selectedPlayerProfileTab==="Achievements"?<View style={s.profileSectionCard}><View style={s.profileSectionHeader}><Text style={s.profileSectionTitle}>ACHIEVEMENTS</Text><Text style={s.profileSeasonYear}>EARNED TROPHIES & BADGES</Text></View>{earnedRows.length?<View>{earnedRows.map((e:any,i:number)=>{const a=mergedAward(e);return <View key={String(e?.id||`${a.type}-${i}`)} style={s.profileAchievementCard}><View style={s.profileAchievementImageWrap}><AwardArtwork award={a} style={s.profileAchievementImage}/><Text style={s.profileAchievementCount}>x{a.count}</Text></View><View style={{flex:1}}><Text style={s.profileAchievementName}>{a.name}</Text><Text style={s.profileAchievementCategory}>{a.category}</Text><Text style={s.profileAchievementLabel}>WHAT IT WAS AWARDED FOR</Text><Text style={s.profileAchievementDetail}>{a.detail}</Text></View></View>})}</View>:<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO ACHIEVEMENTS YET</Text><Text style={s.emptyStateText}>Trophies and badges earned by this player will appear here with the reason each award was earned.</Text></View>}</View>:null}
</ScrollView>;
}else if(tab==="Stats"){
...
const tournament=serverTournamentStats||{};
const tournamentHistory=Array.isArray(serverTournamentHistory)?serverTournamentHistory:[];
body=<ScrollView showsVerticalScrollIndicator={false}>
<Text style={s.title}>My Stats</Text>
<View style={s.statsModeSwitch}><Pressable style={[s.statsModeButton,statsView==="League"&&s.statsModeButtonActive]} onPress={()=>setStatsView("League")}><Text style={[s.statsModeText,statsView==="League"&&s.statsModeTextActive]}>LEAGUE STATS</Text></Pressable><Pressable style={[s.statsModeButton,statsView==="Tournament"&&s.statsModeButtonActive]} onPress={()=>setStatsView("Tournament")}><Text style={[s.statsModeText,statsView==="Tournament"&&s.statsModeTextActive]}>TOURNAMENT STATS</Text></Pressable></View>
{statsView==="League"?<><Text style={s.statsSection}>MATCH RECORD</Text>
...
<View style={s.statsGrid}>{[["3-DART AVERAGE",formatStat(serverSeasonStats.three_dart_average)],["AVG UNTIL 170",formatStat(serverSeasonStats.average_until_170)],["FIRST 9 DART AVG",formatStat(serverSeasonStats.first9_average)],["CHECKOUT %",`${formatStat(serverSeasonStats.checkout_pct,1)}%`],["CHECKOUTS",`${formatStatInt(serverSeasonStats.checkouts_made)}/${formatStatInt(serverSeasonStats.checkout_attempts)}`],["HIGHEST CHECKOUT",formatStatInt(serverSeasonStats.highest_checkout)]].map(x=><View key={x[0]} style={s.statsTile}><Text style={s.statsValue}>{x[1]}</Text><Text style={s.statsLabel}>{x[0]}</Text></View>)}</View>
...
<Text style={s.statsNote}>LIVE FROM LEAGUE SERVER • {formatStatInt(serverSeasonStats.matches_with_stats)} approved league match{Number(serverSeasonStats.matches_with_stats||0)===1?"":"es"} currently contribute detailed dart statistics.</Text></>:<><Text style={s.statsSection}>TOURNAMENT RECORD</Text>
<View style={s.statsGrid}>{[["TOURNAMENTS",formatStatInt(tournament.tournaments_played)],["TITLES",formatStatInt(tournament.tournament_wins)],["MATCHES",formatStatInt(tournament.matches_played)],["WINS",formatStatInt(tournament.matches_won)],["LOSSES",formatStatInt(tournament.matches_lost)],["LEGS WON",formatStatInt(tournament.legs_won)],["LEGS LOST",formatStatInt(tournament.legs_lost)]].map(x=><View key={x[0]} style={s.statsTile}><Text style={s.statsValue}>{x[1]}</Text><Text style={s.statsLabel}>{x[0]}</Text></View>)}</View>
<Text style={s.statsSection}>AVERAGES & CHECKOUTS</Text>
<View style={s.statsGrid}>{[["3-DART AVERAGE",formatStat(tournament.three_dart_average)],["AVG UNTIL 170",formatStat(tournament.average_until_170)],["FIRST 9 DART AVG",formatStat(tournament.first9_average)],["CHECKOUT %",`${formatStat(tournament.checkout_pct,1)}%`],["CHECKOUTS",`${formatStatInt(tournament.checkouts_made)}/${formatStatInt(tournament.checkout_attempts)}`],["HIGHEST CHECKOUT",formatStatInt(tournament.highest_checkout)]].map(x=><View key={x[0]} style={s.statsTile}><Text style={s.statsValue}>{x[1]}</Text><Text style={s.statsLabel}>{x[0]}</Text></View>)}</View>
<Text style={s.statsSection}>3-DART TOTALS</Text>
<View style={s.statsGrid}>{[["60+",formatStatInt(tournament["60_plus"])],["100+",formatStatInt(tournament["100_plus"])],["140+",formatStatInt(tournament["140_plus"])],["170+",formatStatInt(tournament["170_plus"])],["180",formatStatInt(tournament["180s"])]].map(x=><View key={x[0]} style={s.statsTile}><Text style={s.statsValue}>{x[1]}</Text><Text style={s.statsLabel}>{x[0]}</Text></View>)}</View>
<Text style={s.statsSection}>TOURNAMENT HISTORY / BREAKDOWN</Text>
{tournamentHistory.length?tournamentHistory.map((h:any,i:number)=><View key={String(h?.tournament_id||i)} style={s.tournamentStatsHistoryCard}><View style={{flex:1,minWidth:0}}><Text style={s.tournamentStatsHistoryName}>{String(h?.name||"Tournament")}</Text><Text style={s.tournamentStatsHistoryMeta}>{String(h?.placement||h?.status||"Played").replace(/_/g," ").toUpperCase()} • {formatStatInt(h?.matches_played)} MATCH{Number(h?.matches_played||0)===1?"":"ES"} • {formatStatInt(h?.matches_won)}-{formatStatInt(h?.matches_lost)}</Text><Text style={s.tournamentStatsHistoryMeta}>LEGS {formatStatInt(h?.legs_won)}-{formatStatInt(h?.legs_lost)}</Text></View>{h?.champion?<View style={s.tournamentStatsChampionPill}><Text style={s.tournamentStatsChampionText}>CHAMPION</Text></View>:null}</View>):<View style={s.emptyStateCard}><Text style={s.emptyStateTitle}>NO TOURNAMENT STATS YET</Text><Text style={s.emptyStateText}>Approved manager-created tournament matches will appear here without changing your league statistics.</Text></View>}
<Text style={s.statsNote}>TOURNAMENT-ONLY SERVER STATS • Separate from regular-season standings and league/career statistics.</Text></>}
</ScrollView>;}
...
<View style={s.homeTop}><Text style={s.homeWelcomeSmall}>WELCOME BACK,</Text><Text style={s.homeWelcomeBig}>{(currentPlayer?.username||screenName||"PLAYER").toUpperCase()}!</Text><Text style={s.homeDivision}>{currentPlayer?.division_name||"Unassigned"}</Text></View>
...
statsModeSwitch:{flexDirection:"row",backgroundColor:"rgba(4,17,30,.94)",borderWidth:1,borderColor:"#1b3850",borderRadius:12,padding:4,gap:4,marginBottom:14},
statsModeButton:{flex:1,minHeight:44,borderRadius:9,alignItems:"center",justifyContent:"center",paddingHorizontal:8},
statsModeButtonActive:{backgroundColor:"#1477ff"},
statsModeText:{color:"#8ea3b5",fontSize:10,fontWeight:"900",letterSpacing:.5,textAlign:"center"},
statsModeTextActive:{color:"#fff"},
tournamentStatsHistoryCard:{flexDirection:"row",alignItems:"center",gap:10,backgroundColor:"rgba(7,22,36,.84)",borderWidth:1,borderColor:"#1b3850",borderRadius:12,padding:12,marginBottom:8},
tournamentStatsHistoryName:{color:"#fff",fontSize:14,fontWeight:"900"},
tournamentStatsHistoryMeta:{color:"#8ea3b5",fontSize:9,fontWeight:"700",marginTop:3},
tournamentStatsChampionPill:{backgroundColor:"rgba(242,201,76,.16)",borderWidth:1,borderColor:"#f2c94c",borderRadius:10,paddingHorizontal:9,paddingVertical:6},
tournamentStatsChampionText:{color:"#f2c94c",fontSize:8,fontWeight:"900",letterSpacing:.5}
,
...
profileMoreMenu:{backgroundColor:"rgba(3,15,31,.98)",borderWidth:1,borderColor:"rgba(90,150,210,.55)",borderRadius:12,padding:6,marginBottom:12,alignSelf:"flex-end",minWidth:250},
profileMoreMenuItem:{paddingVertical:10,paddingHorizontal:12,borderRadius:9},
profileMoreMenuItemActive:{backgroundColor:"rgba(47,128,237,.18)"},
profileMoreMenuTitle:{color:"#fff",fontSize:11,fontWeight:"900",letterSpacing:.5},
profileMoreMenuSub:{color:"#8ea3b5",fontSize:9,marginTop:3},
profileModeBanner:{backgroundColor:"rgba(47,128,237,.13)",borderWidth:1,borderColor:"rgba(47,128,237,.45)",borderRadius:10,paddingVertical:8,paddingHorizontal:12,marginBottom:10},
profileModeBannerTitle:{color:"#48a4ff",fontSize:12,fontWeight:"900",letterSpacing:.8},
profileModeBannerSub:{color:"#9bb0c2",fontSize:9,marginTop:2},
profileTabButton:{flex:1,alignItems:"center"},
profileTournamentHistoryRow:{flexDirection:"row",alignItems:"center",paddingVertical:11,borderTopWidth:1,borderColor:"rgba(255,255,255,.10)"},
profileTournamentName:{color:"#fff",fontSize:12,fontWeight:"900"},
profileTournamentMeta:{color:"#8ea3b5",fontSize:9,marginTop:3},
profileTournamentPlacement:{color:"#3ea4ff",fontSize:9,fontWeight:"900",marginLeft:8},
profileAchievementCard:{flexDirection:"row",alignItems:"center",paddingVertical:12,borderTopWidth:1,borderColor:"rgba(255,255,255,.10)"},
profileAchievementImageWrap:{width:74,alignItems:"center",justifyContent:"center",marginRight:10},
profileAchievementImage:{width:58,height:58},
profileAchievementCount:{color:"#fff",fontSize:10,fontWeight:"900",marginTop:2},
profileAchievementName:{color:"#fff",fontSize:14,fontWeight:"900"},
profileAchievementCategory:{color:"#3ea4ff",fontSize:9,fontWeight:"800",marginTop:2},
profileAchievementLabel:{color:"#8ea3b5",fontSize:8,fontWeight:"900",letterSpacing:.5,marginTop:6},
profileAchievementDetail:{color:"#d6e1ea",fontSize:10,lineHeight:15,marginTop:3},
...
profileBigName:{color:"#fff",fontSize:28,fontWeight:"900",letterSpacing:.5,marginBottom:2},profileRealName:{color:"#8ea3b5",fontSize:12,fontWeight:"700",marginBottom:8},
...
profileTab:{color:"#7f8b9a",fontSize:10,fontWeight:"900",paddingVertical:10,textAlign:"center"},
